...
Galv.JA.stopperEvent = function(x,y) {
	...
	for (var i = 0;i <= events.length;i++) {
		if (events[i] && events[i].page()) {
			...
			console.log(events[i].page)
			console.log(events[i].page().list[0])

			if (events[i].page().list[0].code === 356 && events[i].page().list[0].parameters[0] === "<blockJump>") {
				block = true;
				console.log("supposed to block?");
				break;
			};
		};
	};
